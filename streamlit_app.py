...
with tabs[2]:
  ...
  if st.checkbox('Mostrar modelo de regresión logística', key=next(widget_id)):
    ...
    x_pred = np.linspace(x_min, x_max, 50)  # Time points for prediction
    ...
    input_value = st.number_input(f'Introduce un valor de X', min_value=float(df_3['Tiempo_dias'].min()), step=0.1, value=df_3['Tiempo_dias'].min(), key=next(widget_id))
